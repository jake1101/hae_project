...
(function (global, factory) {
	...
	(global = global || self, global.ZipLoader = factory());
	...
	*/
	...
	var CENTRAL_DIRECTORY = 0x02014b50; // const END_OF_CENTRAL_DIRECTORY = 0x06054b50;

	var parseZip = function parseZip(buffer) {
	  var reader = new _default(buffer);
	  var files = {};
	...
	var parseLocalFile = function parseLocalFile(reader) {
	  ...
	  reader.skip(extraFieldLength); // for ( i = 0; i < extraFieldLength; i ++ ) {
	  ...
	  switch (compression) {
	    ...
	    case 8:
	      data = new Uint8Array(inflate_1$1.inflate(compressedData, {
	        raw: true
	      }));
	      break;

	    default:
	      console.log("".concat(filename.join(''), ": unsupported compression type"));
	      data = compressedData;
	  }
	...
	var parseCentralDirectory = function parseCentralDirectory(reader) {
	  // let i = 0;
	  reader.skip(24); // const versionMadeby        = reader.readBytes( 2 );
	  ...
	  reader.skip(12); // const diskNumberStart      = reader.readBytes( 2 );
	  ...
	  reader.skip(filenameLength); // for ( i = 0; i < filenameLength; i ++ ) {
	  // 	filename.push( String.fromCharCode( reader.readBytes( 1 ) ) );
	  // }
	  ...
	  reader.skip(fileCommentLength); // for ( i = 0; i < fileCommentLength; i ++ ) {
