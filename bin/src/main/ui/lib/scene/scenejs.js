...
//        body.appendChild(div);
    ...
    this.taskFinished = function (taskId) {
        ...
        /*console.log(task)
        console.log(new Date() + "task finish task finish task finish task finish task finish task finish task finish")*/
 ...
 * @param handle Publication handle
 */
SceneJS.Node.prototype.off = function (handle, callBack) {
    var topic = this._handleTopics[handle];
    if (topic) {
        delete this._handleTopics[handle];
        var topicSubs = this._topicSubs[topic];
...
SceneJS_NodeFactory.prototype._getType = function (engine, typeName, ok) {
    ...
    var taskId = SceneJS_sceneStatusModule.taskStarted(engine.scene, "Loading plugin  "+typeName);
    subs.push(function () {
...
new (function () {
    ...
    var defaultCore = {
        ...
        checkAspect: function (core, aspect) {
    		if (core.optics.aspect && aspect && core.optics.aspect != aspect) {
...
//                        });
    ...
    SceneJS.TextureMap.prototype._loadTexture = function (src, preloadSrc) {
        var self = this;
        var taskId = SceneJS_sceneStatusModule.taskStarted(this, "Loading texture  " + src);
        
        var image = null;
        var existing = false;
        //hys
        if(src.indexOf("ship_left")!=-1){
        	image = image_ship_left;
        	existing = true;
        }else if(src.indexOf("ship_right")!=-1){
        	image = image_ship_right;
        	existing = true;
        }else if(src.indexOf("cloudySea")!=-1){
        	image = image_cloudySea;
        	existing = true;
        }else if(src.indexOf("grimmNight")!=-1){
        	image = image_grimmNight;
        	existing = true;
        }
        else{
        	image = new Image();
...
(function () {
    ...
    SceneJS_Display.prototype.pick = function (params) {
        ...
        /*	debugger;
        	...
        	console.log('canvasPos[0] : '+ canvasPos[0]);
        	console.log('canvasPos[1] : '+ canvasPos[1]);
        	...
        	*/
            canvasPos = new Float32Array([(canvasPos[0]-canvas.getBoundingClientRect().left)* resolutionScaling, (canvasPos[1]-canvas.getBoundingClientRect().top)* resolutionScaling]);
        }
