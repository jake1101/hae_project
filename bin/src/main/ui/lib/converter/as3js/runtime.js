(function() {
  ...
  Program["com.mcleodgaming.as3js.Main"] = function(module, exports) {
    ...
    var AS3Parser;
    module.inject = function() {
      AS3Parser = module.importObj('com.mcleodgaming.as3js.parser', 'AS3Parser');
...
//      var mainTemplate = "(function(){var Program={};{{packages}}if(typeof module !== 'undefined'){module.exports=AS3JS.load({program:Program,entry:\"{{entryPoint}}\",entryMode:\"{{entryMode}}\"});}else if(typeof window!=='undefined'&&typeof AS3JS!=='undefined'){window['{{entryPoint}}']=AS3JS.load({program:Program,entry:\"{{entryPoint}}\",entryMode:\"{{entryMode}}\"});}})();";
      var mainTemplate = "{{packages}}";
      var classTemplate = "{{source}}";
//      var classTemplate = "Program[\"{{module}}\"]=function(module, exports){{{source}}};";
  ...
  Program["com.mcleodgaming.as3js.parser.AS3Class"] = function(module, exports) {
    ...
    module.inject = function() {
      Main = module.importObj('com.mcleodgaming.as3js', 'Main');
      AS3Parser = module.importObj('com.mcleodgaming.as3js.parser', 'AS3Parser');
      AS3Pattern = module.importObj('com.mcleodgaming.as3js.enums', 'AS3Pattern');
      AS3Function = module.importObj('com.mcleodgaming.as3js.types', 'AS3Function');
      AS3Variable = module.importObj('com.mcleodgaming.as3js.types', 'AS3Variable');
      AS3Class.reservedWords = ["as", "class", "delete", "false", "if", "instanceof", "native", "private", "super", "to", "use", "with", "break", "const", "do", "finally", "implements", "new", "protected", "switch", "true", "var", "case", "continue", "else", "for", "importObj", "internal", "null", "public", "this", "try", "void", "catch", "default", "extends", "function", "in", "is", "package", "return", "throw", "typeof", "while", "each", "get", "set", "namespace", "include", "dynamic", "final", "natiev", "override", "static", "abstract", "char", "export", "long", "throws", "virtual", "boolean", "debugger", "float", "prototype", "to", "volatile", "byte", "double", "goto", "short", "transient", "cast", "enum", "intrinsic", "synchronized", "type"];
    ...
    AS3Class.prototype.stringifyFunc = function(fn) {
      ...
      if (fn instanceof AS3Function) {
        //fn.value = fn.convertComment(fn.value);
    ...
    AS3Class.prototype.process = function(classes) {
      ...
      for (i in this.setters) {
        if (this.setters[i] instanceof AS3Function) {
          this.setters[i].value = this.setters[i].convertComment(this.setters[i].value);
          allFuncs.push(this.setters[i]);
        }
      }
      for (i in this.staticSetters) {
        if (this.staticSetters[i] instanceof AS3Function) {
          this.staticSetters[i].value = this.staticSetters[i].convertComment(this.staticSetters[i].value);
          allStaticFuncs.push(this.staticSetters[i]);
        }
      }
      for (i in this.getters) {
        if (this.getters[i] instanceof AS3Function) {
          this.getters[i].value = this.getters[i].convertComment(this.getters[i].value);
          allFuncs.push(this.getters[i]);
        }
      }
      for (i in this.staticGetters) {
        if (this.staticGetters[i] instanceof AS3Function) {
          this.staticGetters[i].value = this.staticGetters[i].convertComment(this.staticGetters[i].value);
          allStaticFuncs.push(this.staticGetters[i]);
        }
      }
      for (i in this.members) {
        if (this.members[i] instanceof AS3Function) {
          this.members[i].value = this.members[i].convertComment(this.members[i].value);
          allFuncs.push(this.members[i]);
        }
      }
      for (i in this.staticMembers) {
        if (this.staticMembers[i] instanceof AS3Function) {
          this.staticMembers[i].value = this.staticMembers[i].convertComment(this.staticMembers[i].value);
...
/*
      ...
      if (this.parentDefinition) {
        buffer += "var " + this.parentDefinition.className + " = module.importObj('" + this.parentDefinition.packageName + "', '" + this.parentDefinition.className + "');\n";
      }
      ...
      for (i in this.imports) {
        if (!(this.ignoreFlash && this.imports[i].indexOf('flash.') >= 0) && this.packageName + '.' + this.className != this.imports[i] && !(this.parentDefinition && this.parentDefinition.packageName + '.' + this.parentDefinition.className == this.imports[i])) //Ignore flash imports and parent for injections
        {
          // Must be in the filtered map, otherwise no point in writing
          if (this.classMapFiltered[this.packageMap[this.imports[i]].className]) {
            injectedText += "\t" + this.imports[i].substr(this.imports[i].lastIndexOf('.') + 1) + " = module.importObj('" + this.packageMap[this.imports[i]].packageName + "', '" + this.packageMap[this.imports[i]].className + "');\n";
          }
        }
      }
...
*/
      ...
      // SMARTsuite9 script properties
      // variable, fucntion - class 및 prototype 제거
      buffer += "properties: {\n\n";
      ...
      if (this.staticMembers.length > 0) {
        ...
        for (i in this.staticMembers) {
          if (this.staticMembers[i] instanceof AS3Function) {
            buffer += this.stringifyFunc(this.staticMembers[i]);
//            buffer += this.className + "." + this.stringifyFunc(this.staticMembers[i]);
            ...
            if (isNaN(parseInt(this.staticMembers[i].value))) {
              buffer += this.staticMembers[i].name + ' : ' + this.staticMembers[i].value + ",";
//              buffer += this.className + "." + this.stringifyFunc(this.staticMembers[i]);
            } else {
              buffer += this.stringifyFunc(this.staticMembers[i]);
//              buffer += this.className + "." + this.stringifyFunc(this.staticMembers[i]);
...
//            buffer += this.className + "." + this.staticMembers[i].name + ' = null;\n';
          ...
          buffer += ' // [original] '+this.staticMembers[i].encapsulation+' static var '+this.staticMembers[i].name+':'+this.staticMembers[i].type+'='+this.staticMembers[i].value+'\n\n';
        }
        for (i in this.staticGetters) {
          buffer += this.stringifyFunc(this.staticGetters[i]);
//          buffer += this.className + "." + this.stringifyFunc(this.staticGetters[i]);
        }
        for (i in this.staticSetters) {
          buffer += this.stringifyFunc(this.staticSetters[i]);
//          buffer += this.className + "." + this.stringifyFunc(this.staticSetters[i]);
      ...
      // variable
      var prop={},
          varLength=(function(_members){
            var retVal = 0;
            _members.filter(function(member){
              if(member instanceof AS3Variable){
                retVal += 1;
              }
            });
            return retVal;
          ...
          propertiesFormat=function(type, value){
            return '{\n' + 
                    ' type : '+type+',\n'+
                      ' \t value : function(){\n'+
                        ' \t\t return '+value+ ';'+
      ...
      for (i in this.members) {
            prop = this.members[i];
          if (prop.name === this.className) {
            continue;
          }
//          if (this.members[i] instanceof AS3Function || (AS3Class.nativeTypes.indexOf(this.members[i].type) >= 0 && this.members[i].value)) {
          if (prop instanceof AS3Function) {

            //prop.value = prop.convertComment(prop.value);

          } else if(prop instanceof AS3Variable){
          varLength -= 1;
          
            if (prop.type === "Number" || prop.type === "int" || prop.type === "uint") {
                if (isNaN(parseInt(this.members[i].value))) {
                  buffer += prop.name + ' : '+propertiesFormat("Number", "");
    //              buffer += this.className + ".prototype." + this.members[i].name + ' = 0;\n';
                } else {
                	buffer += prop.name + ' : '+propertiesFormat("Number", prop.value);
    //              buffer += this.className + ".prototype." + this.stringifyFunc(this.members[i]);
                }
            } else if (prop.type === "Boolean") {
                buffer += prop.name + ' : ' + propertiesFormat(prop.type, prop.value);
    //            buffer += this.className + ".prototype." + this.members[i].name + ' = false;\n';
            } else if (prop.type === "String") {
                buffer += prop.name + ' : ' + propertiesFormat(prop.type, '""');
    //            buffer += this.className + ".prototype." + this.members[i].name + ' = false;\n';
            } else if (prop.type === "SCObject" || prop.type === "Object") {
                buffer += prop.name + ' : '+propertiesFormat("Object", "{}");
            } else if (prop.type === "SCCollection") {
                buffer += prop.name + ' : '+propertiesFormat("Array", "[]");
            } else {
                buffer += prop.name + ' : {}';
    //            buffer += this.className + ".prototype." + this.members[i].name + ' = null;\n';
            }
            
            buffer += hasNextFn(varLength);
            buffer += ' // [original] var '+prop.name+':'+prop.type+'='+prop.value+'\n\n';
...
//          buffer += this.members[i].name + ' = false;\n';
////          buffer += this.className + ".prototype." + this.members[i].name + ' = false;\n';
//        } else {
//          buffer += this.members[i].name + ' : null,\n';
////          buffer += this.className + ".prototype." + this.members[i].name + ' = null;\n';
//        }
      }
...
//      buffer += "module.exports = " + this.className + ";\n";
  ...
  Program["com.mcleodgaming.as3js.parser.AS3Parser"] = function(module, exports) {
    ...
    module.inject = function() {
      Main = module.importObj('com.mcleodgaming.as3js', 'Main');
      AS3Class = module.importObj('com.mcleodgaming.as3js.parser', 'AS3Class');
      AS3Token = module.importObj('com.mcleodgaming.as3js.parser', 'AS3Token');
      AS3Encapsulation = module.importObj('com.mcleodgaming.as3js.enums', 'AS3Encapsulation');
      AS3MemberType = module.importObj('com.mcleodgaming.as3js.enums', 'AS3MemberType');
      AS3ParseState = module.importObj('com.mcleodgaming.as3js.enums', 'AS3ParseState');
      AS3Pattern = module.importObj('com.mcleodgaming.as3js.enums', 'AS3Pattern');
      AS3Argument = module.importObj('com.mcleodgaming.as3js.types', 'AS3Argument');
      AS3Function = module.importObj('com.mcleodgaming.as3js.types', 'AS3Function');
      AS3Member = module.importObj('com.mcleodgaming.as3js.types', 'AS3Member');
      AS3Variable = module.importObj('com.mcleodgaming.as3js.types', 'AS3Variable');
    ...
    AS3Parser.checkArguments = function(fn) {
      ...
      for (var i = 0; i < fn.argList.length; i++) {
        ...
        if (fn.argList[i].isRestParam) {
          ...
          args += "\n\t\t\t" + fn.argList[i].name + " = " + fn.argList[i].name + " || " + fn.argList[i].value+";";
        }
      }
      return fn.value.substr(0, start + 1) + "\n/**"+ args+"" + fn.value.substr(start + 1);
    };
    ...
    AS3Parser.prototype.parseHelper = function(cls, src) {
      ...
      for (index = 0; index < src.length; index++) {
        ...
        if (this.getState() == AS3ParseState.START) {
          ...
          } else if (currToken.token == 'importObj') {
  ...
  Program["com.mcleodgaming.as3js.types.AS3Argument"] = function(module, exports) {
    var AS3Variable = module.importObj('com.mcleodgaming.as3js.types', 'AS3Variable');
  ...
  Program["com.mcleodgaming.as3js.types.AS3Function"] = function(module, exports) {
    var AS3Member = module.importObj('com.mcleodgaming.as3js.types', 'AS3Member');
    var AS3Variable;
    module.inject = function() {
      AS3Variable = module.importObj('com.mcleodgaming.as3js.types', 'AS3Variable');
    };
    ...
    AS3Function.prototype.convertComment = function(value){
      // 1. /**를 /*로 치환
      ...
      return (function(v){
        var startIdx, endIdx;
        startIdx = v.indexOf('{')+1;
        v = v.substr(0, startIdx) + v.substr(startIdx);
        endIdx = v.lastIndexOf('}');
        v = v.substr(0, endIdx) + '*/\n' + v.substr(endIdx);
        return v;
      })(value);
    }
  ...
  if (typeof module !== 'undefined') {	  
    ...
    this.as3 = window['com.mcleodgaming.as3js.Main'] = AS3JS.load({
      program: Program,
      entry: "com.mcleodgaming.as3js.Main",
      entryMode: "static"
    });
