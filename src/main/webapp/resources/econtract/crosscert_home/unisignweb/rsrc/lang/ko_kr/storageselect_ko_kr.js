(function() {
	return {
		IDS_STORAGE_SELECTION: 					"저장매체 선택",
		IDS_STORAGE_SELECTION_CLOSE: 			"저장매체 선택창 종료",
		IDS_STORAGE_REMOVABLE: 					"이동식디스크",
		IDS_STORAGE_SECTOKEN: 					"보안토큰",
		IDS_STORAGE_SAVETOKEN: 					"저장토큰",
		IDS_STORAGE_MOBILEPHONE: 				"휴대폰",
		IDS_STORAGE_HARDDISK: 					"하드디스크",
		// nhkim 20151015
		IDS_STORAGE_MOBILETOKEN: 				"스마트인증",
		IDS_STORAGE_SECUREDISK: 				"안전디스크",		
		//		
		IDS_STORAGE_LS:                         "웹스토리지",
		IDS_STORAGE_SS:							"공유스토리지",		
		IDS_STORAGE_TOUCHSIGN :                 "터치싸인",
		IDS_STORAGE_SMARTSIGN :                 "스마트인증",
		IDS_STORAGE_WEBSECTOKEN :               "웹보안토큰",
		IDS_STORAGE_WEBSOFTTOKEN :              "소프트보안토큰",
		IDS_STORAGE_CLOUDSIGN :              	"클라우드 사인",		        		
		IDS_STORAGE_ETC :                       "미지원매체",
	
		IDS_BUTTON: 							"버튼",
		IDS_CONFIRM: 							"확인",
		IDS_CLOSE: 								"종료",
		IDS_CANCEL: 							"취소",
		IDS_WARNING: 							"공인인증서를 PC 하드디스크에 저장하는 경우 공인인증서 유출 위험이 있습니다. 보안토큰, 이동식 디스크 등의 저장매체를 권고합니다.",
		IDS_SAVE_NOTICE: 						"인증서를 저장할 매체를 선택해주세요.",
		IDS_COPY_NOTICE: 						"인증서를 복사할 매체를 선택해주세요.",
		
		IDS_SAVETOKEN_SMART_CARD: 				"스마트카드",
		IDS_SAVETOKEN_USB_TOKEN: 				"USB 토큰",
		
		IDS_MSGBOX_ERROR_PLEASE_SELECT_STORAGE: "저장매체를 선택해 주십시오.",
		IDS_MSGBOX_ERROR_WARNING_SAME_STORAGE: 	"원본 저장매체와 복사하려는 대상매체가 동일합니다. 대상매체를 다시 선택해 주십시오.",
		IDS_MSGBOX_PLUGIN_ERROR_UNLOAD:         "UniSignWeb Plug-In 이 장시간 미사용 등의 이유로 언로드 되었습니다.\n페이지 새로 고침을 하시거나 첫 페이지로 이동하셔서 재시도 해주십시오.",
		IDS_MSGBOX_NOT_SUPPORTED_MEDIA: 		"지원되지 않는 저장매체입니다.",
		
		IDS_CONFIRMBOX_WARNING_CHANGE_CERT:     "스마트카드에 이미 인증서가 있는 경우 덮어 써집니다.\n계속 진행하시겠습니까?",
		// nhkim 20151016
		IDS_MSGBOX_NIM_ERROR_UNLOAD: 			"UniCRSLocalServer가  언로드 되었습니다.\n페이지 새로 고침을 하시거나 첫 페이지로 이동하셔서 재시도 해주십시오.",
		IDS_CMSGBOX_NOT_INSTALL_SD: 			"안전디스크 모듈이 설치되지 않았습니다.\n설치 후에 서비스 이용하시기 바랍니다.",
		IDS_CONFIRMBOX_NOT_INSTALL_SD: 			"안전디스크 모듈이 설치되지 않았습니다.\n안전디스크 모듈 설치 페이지로 이동하시겠습니까?",
		IDS_CONFIRMBOX_NOT_INSTALL_SMARTCERT: 	"스마트인증 모듈이 설치되지 않았습니다.\n스마트인증 모듈 설치 페이지로 이동하시겠습니까?",
		IDS_MSGBOX_NOT_INSTALL_SMARTCERT: 		"스마트인증 모듈이 설치되지 않았습니다.\n스마트인증 모듈 설치 페이지로 이동하시기 바랍니다.",		
		IDS_MSGBOX_NOT_INSTALL_MOBILE: 			"휴대폰 공인인증 서비스 모듈이  설치가 되지 않았습니다.\n설치 후에 서비스 이용하시기 바랍니다.",
		IDS_CONFIRMBOX_NOT_INSTALL_MOBILE: 		"휴대폰 공인인증 서비스 모듈이  설치가 되지 않았습니다.\n설치페이지로 이동하시겠습니까?",
				
		IDS_MSGBOX_NOT_INSERT_BACKUPEDDISK: 	"인증서가 백업된 이동식 디스크가 연결되어 있지 않습니다.\n이동식 디스크를 연결하시고 다시 진행해 주시기 바랍니다.",
	    
	    IDS_MSGBOX_BLOCK_POPUP_WINDOW:						"설치페이지 open 실패했습니다.\n팝업이 차단되어 있는지 확인하시기 바랍니다."		 
		
	}
})();
